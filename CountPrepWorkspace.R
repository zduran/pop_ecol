...
# Answer: This will link the predictor to the observation data,
# first dataset = right, second = left
...
# Answers:
# Sites with 0 count (unoccupied) have been increasing over time. Counts have
# been decreasing over time. This reflects results from Week 5 analysis.
# (i.e., evidence of extinction model)
...
write.csv( opendf, paste( getwd(),"/simdata/open_counts.csv", sep = "" ),  
           row.names = FALSE )

#save closed dataframe in our data folder:
write.csv( closeddf, paste( getwd(),"/simdata/closed_counts.csv", sep = "" ),  
           row.names = FALSE )
