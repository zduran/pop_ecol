...
preddf <- preddf %>% filter( marked == "yes" & year == 2013 ) %>% 
  select( -counted, -marked, -yearname )
# create scaled predictor dataframe: 
sc <- apply( preddf[ ,c("cheatgrass", "sagebrush")],
             MARGIN = 2, FUN = scale )
# function 'scale' calculates the mean of the values, subtracts from actual and
# divides by one SD; you would divide by two SD if predictors are a combo of
# continuous and categorical predictors; MARGIN = 2 means we are scaling the 
# columns (2) versus the rows (1)
...
CH
# 7 unique capture histories

#make closed dataframe
closed_df <- open_df %>% filter( year == 2013 )
#check
...
jMat <- matrix( c("a","b","c"), M, J, byrow = TRUE )
jMat
...
umf.2013.Mt <- unmarkedFrameMPois( y = y.closed,
                                   siteCovs = sitepreds,
                                   #define survey id:
                                   obsCovs = list( J = jMat ),
...
#Answer: 
# the temp predictors were average min/max per year (within year no variation)

#check
summary(umf.2013.Mt)

#define unmarked dataframe for M[b] and M[o] for single season
umf.2013.Mb <- unmarkedFrameMPois( y = y.closed,
                                   siteCovs = sitepreds,
...
#Answer:
# Initially, this shows cheatgrass has a negative effect on abundance and 
# sagebrush has a similar positive effect on abundance; the addition of predictors
# reduced the AIC from AIC: 2214.748 (null) to AIC: 2145.692 (site-level pred.)
...
# Answer: 
# the fitlist function cannot use two different data structures, but comparing 
# the AICs are perfectly appropriate

#compare
modSel( closedmodels )
# Which is our top model?
# Answer:
# For models we can compare, adding the time-level differences with the site-level
# predictors performs best. However, in comparing behavior model to Mt, Mb has lower
# AIC.
...
# Answer: There are very little differences between the predicted and actual data
# the distribution chosen likely captures the variation
...
# note: sagebrush = 0 means set sagebrush to the mean (since the predictor was 
# scaled)
...
# Answer: as cheatgrass percent cover increases, relative abundance decreases
...
sagep
# How do you interpret this relationship?
# Answer: as percent sagebrush increases, relative abundance increases
#
...
# Answer: After the first detection, probability of detection higher for wise 
# individuals (i.e., evidence of trap-happy behavior)
#
########################################################################
...
tiff( 'simdata/TrapBehavior.tiff',
