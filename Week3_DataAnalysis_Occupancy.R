...
# load packages:
library( tidyverse )
#includes dplyr, tidyr and ggplot2
library( unmarked ) 
# note - cannot use random effects; sister package 'ubms' for occupancy data)
library( MuMIn )
## end of package load ###############
###################################################################
#### Load or create data -----------------------------------------
...
# easy
# NOTE: 'y' = MxJ matrix where M = rows (number of sites) and J = columns (obvs
# per site)
...
# Answer:
# Predictors are often collected on different scales, this allows standardization
...
fm.closed <- occu( ~ 1 + obsv + sagebrush
                   ...
                   data = umf )
# top line = "detection is a function(~) of an intercept(1) + observer effect + 
#             sagebrush"
# second line = "occupancy is a function (~) of an intercept + sagebrush +
#               cheatgrass"
...
#MODEL RESULTS#
# Call:
#   occu(formula = ~1 + obsv + sagebrush ~ 1 + sagebrush + cheatgrass, 
#        data = umf)
# 
# Occupancy:
#             Estimate    SE    z   P(>|z|)
# (Intercept)   -1.182 0.282 -4.19 2.81e-05
# sagebrush      0.892 0.285  3.13 1.77e-03 #positive effect to occu#
# cheatgrass    -0.855 0.324 -2.64 8.37e-03 #neg effect to occu#
# 
# Detection:
#             Estimate    SE     z  P(>|z|)
# (Intercept)    2.292 0.763  3.01 0.00265  #Note this is tech.1#
# obsvtech.2    -2.489 0.930 -2.68 0.00747
# obsvtech.3    -1.081 0.880 -1.23 0.21933
# obsvtech.4    -1.717 0.868 -1.98 0.04791
# sagebrush     -0.213 0.187 -1.14 0.25430  #more sagebrush harder to see sq.#
# 
# AIC: 198.2429 
...
# Answer:
# Predictor is the variable, coefficient (effect size) is the direction of 
# influence
...
# We use unmarked function model() to compare models using AIC:
unmarked::modSel(fms )
...
# psi(sagebrush + cheatgrass)p(obsv)            7     197.54 0.00  4.9e-01 0.49
# psi(sagebrush + cheatgrass)p(obsv+sagebrush)  8     198.24 0.70  3.4e-01 0.83
#
